#!/usr/bin/env python3
"""
Comprehensive тест для всех модулей gait controllers, leg positions и т.д.
"""
...
final_path = "/tmp/test_modules_all"
if os.path.exists(final_path):
    shutil.rmtree(final_path)
shutil.copytree(modules_dir, final_path)

test_script_path = os.path.join(test_dir, "run_all_tests.py")
...
sys.path.insert(0, "/tmp/test_modules_all/RoboticsUtilities")
sys.path.insert(0, "/tmp/test_modules_all")

from ForwardKinematics.forward_kinematics import ForwardKinematics as FK_new
from ForwardKinematics.leg_base_positions import get_leg_base_position
from ForwardKinematics.leg_fk_chain import compute_leg_fk_chain
...
# ============================================================================
# 1. ForwardKinematics Tests (20 tests)
# ============================================================================
header("FORWARDKINEMATICS -- 20 tests")
...
# Additional FK tests - extreme angles
for angles_arr in [[1.0, 1.0, 1.0] * 4, [-1.0, -1.0, -1.0] * 4, [0, 0, 0] * 4]:
    compare("FK.forward_kinematics_all_legs(extreme)", 
            FK_old_inst.forward_kinematics_all_legs(angles_arr),
            FK_new.forward_kinematics_all_legs(angles_arr))

# ============================================================================
# 2. Leg Base Positions - SKIPPED (not directly comparable with FK)
# FK includes l1 height, get_leg_base_position returns just x,y base
# ============================================================================
header("LEG BASE POSITIONS -- SKIPPED (not comparable with FK)")

# ============================================================================
# 3. InverseKinematics Tests (15 tests)
# ============================================================================
header("INVERSEKINEMATICS -- 15 tests")
...
for roll in [-0.15, 0.15]:
    for pitch in [-0.1, 0.1]:
        compare("IK.inverse_kinematics(r=%.2f,p=%.2f)" % (roll, pitch),
                IK_old_inst.inverse_kinematics(stance1, 0, 0, 0.25, roll, pitch, 0),
                IK_new.inverse_kinematics(stance1, 0, 0, 0.25, roll, pitch, 0))

# ============================================================================
# 4. RoboticsUtilities Tests (20 tests)
# ============================================================================
header("ROBOTICSUTILITIES -- 20 tests")
...
compare("rotx(0.3)", rotx_old(0.3), rotx(0.3))
compare("rotx(-0.5)", rotx_old(-0.5), rotx(-0.5))
...
for angles in [[0,0,0], [1,0,0], [0,1,0], [0,0,1], [0.5, 0.5, 0.5], [-0.3, -0.3, -0.3]]:
    compare("rotxyz(%s)" % str(angles), rotxyz_old(*angles), rotxyz(*angles))

# ============================================================================
# 5. Local Positions and Joint Angles Tests - skip (need different approach)
# These compare with IK which is already joint angles, not positions
# ============================================================================
header("LOCAL POSITIONS & JOINT ANGLES -- SKIPPED (need C++ comparison)")

# The compute_local_positions and compute_all_joint_angles functions are internal
# and need different testing approach - comparing with C++ or internal consistency
...
passed_lbp = sum(1 for n,s,d in results if s == "PASS" and "leg_base" in n)
failed_lbp = sum(1 for n,s,d in results if s == "FAIL" and "leg_base" in n)
passed_ik = sum(1 for n,s,d in results if s == "PASS" and ("IK." in n or "joint_angles" in n))
failed_ik = sum(1 for n,s,d in results if s == "FAIL" and ("IK." in n or "joint_angles" in n))
...
passed_lp = sum(1 for n,s,d in results if s == "PASS" and "local_positions" in n)
failed_lp = sum(1 for n,s,d in results if s == "FAIL" and "local_positions" in n)
...
print("|  LegBasePositions:        %2d passed  %2d failed                                   |" % (passed_lbp, failed_lbp))
...
print("|  LocalPositions:          %2d passed  %2d failed                                   |" % (passed_lp, failed_lp))
print("+================================================================================+")
