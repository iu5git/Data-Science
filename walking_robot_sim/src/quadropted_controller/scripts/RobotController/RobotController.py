...
class Robot:
    def __init__(self, node, body, legs, imu, robot_id):
        ...
        self.crawlGaitController = CrawlGaitController(
            self.default_stance, stance_time=0.55, swing_time=0.45, time_step=0.02
        )

        self.standController = StandController(
        ...
        self.node.create_subscription(
            RobotModeCommand, "robot_mode", self.mode_callback, 10
        )
        self.node.create_subscription(
        ...
        if msg.robot_id == self.robot_id:
            if self.node.verbose:
                self.node.get_logger().info(
                    f"Received mode command: {msg.mode} for robot_id: {msg.robot_id}"
                )
            if msg.mode == "REST":
