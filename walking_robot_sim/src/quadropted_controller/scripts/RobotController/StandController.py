...
class StandController:
    ...
    def run(self, state, command):
        ...
        state.body_local_orientation[0] += (
            angular_vel[0] * self.body_angular_scale
        )  # Roll
        state.body_local_orientation[1] += (
            angular_vel[1] * self.body_angular_scale
        )  # Pitch
        state.body_local_orientation[2] += (
            angular_vel[2] * self.body_angular_scale
        )  # Yaw

        if self.node.verbose:
            self.node.get_logger().info(
                f"Updated body position: {state.body_local_position}"
            )
            self.node.get_logger().info(
                f"Updated body orientation: {state.body_local_orientation}"
            )
